...
export class AuthService implements CanActivate, CanActivateChild, CanLoad {
  ...
  public auth(provider:string, authConfig: any):void{
    ...
    if(provider == "facebook" && !this.isLoggedIn()){ 
       ...
       authConfig.facebook.clientId+'&redirect_uri='+authConfig.facebook.redirectURI+'&scope=email';
    }
    if(provider == "google" && !this.isLoggedIn()){ 
       ...
       authConfig.google.clientId+'&redirect_uri='+authConfig.google.redirectURI+'&scope=email%20profile';
    }
    else{
        this.router.navigate([this.cachedURL]);
    }
  ...
  basicLogin(email: string, password: string) {
      let backend = `${environment.domain_back}`;

      return this._http.post(`${backend}/api/public/auth/login`, { email: email, password: password })
  ...
  faceLogin(password: string, faceId: string) {
    let backend = `${environment.domain_back}`;

    return this._http.post(`${backend}/api/public/auth/login/face`, { password: password, faceId: faceId })
        .map((response: Response) => {
            localStorage.setItem('isLoggedIn', "true");
            localStorage.setItem('token', response.json().token);
            return response.json;
